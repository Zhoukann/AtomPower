...
top_module_name = 'BlockDeviceController'
design_name = 'BlockDeviceModelRocket'
        ...
        if index_type == '':
            for i in range(self.width):
                ...
                for j in range(self.depth):
                    ...
                    if  0 <= i < len(self.connection_nodes):

                        if  0 <=  j < len(self.connection_nodes[i]):
                            self.connection_nodes[i][j].add_input_connection(li)
                        #else:
                            #print(f"[Warning: j = {j}, ? {len(self.connection_nodes[i1])}] ")
                
        elif index_type == 'index':

            index1 = i1-self.widthoffset

            if 0 <= index1 < len(self.connection_nodes):

                if 0 <= i2 < len(self.connection_nodes[index1]):

                    self.connection_nodes[i1-self.widthoffset][i2].add_input_connection(l)

                #else:
                    #print(f"[Warning: i2 = {i2}, ? {len(self.connection_nodes[index1])}] ")
            ...
            for i in range(self.width):
                ...
                for j in range(self.depth):
                    ...
                    if  0 <= i < len(self.connection_nodes):
                        ...
                        #else:
                            p#rint(f"[Warning: j = {j}, ? {len(self.connection_nodes[i1])}] ")
